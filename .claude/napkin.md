...
2. **[2026-07-30] Fórmulas de coleção precisam do `deriveAll`**
   Do instead: Calcular `AVERAGE`, `SUM` e proporções entre jogadores via `collectionMetrics`; `MetricDefinition.formula` enxerga somente uma linha.

3. **[2026-07-30] Presets devem apontar para keys reais da posição**
   Do instead: Executar `presets.test.ts` ao alterar métricas; os módulos misturam camelCase e snake_case e aliases silenciosos quebram o score.

4. **[2026-03-24] pf() é crítico para parsing de números PT-BR**
...
5. **[2026-03-24] Implementação em worktree separada**
...
4. **[2026-07-30] `1.000` depende da coluna**
   Do instead: Aplicar `normalizeWorkbookRow` antes de `pf()`: nos alvos das macros é decimal (1,0); em Minutos continua sendo milhares (1000).
